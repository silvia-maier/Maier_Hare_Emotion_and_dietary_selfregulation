function GLM_FC(sub)
% fMRI first-level analysis for the subjective food value model
...
% orthogonalize or not? (orthogonalize = 1 de-means pmods)
orth_switch = 1; % 0 = do not orthogonalize pmods, 1 = orthogonalize pmods

% include onset only if it has at least 4 events, otherwise estimation becomes too noisy
min_event_num = 4;
...
% onsets
ons_name{1} = 'choice'; % any choice made

% get values
% load design file (to get food image ID)
imglist = load([behavior_path 'imagelist_' sub '.mat']); 

% weighted subjective food values were created with weighted_food_value.m
stimval = load([behavior_path '/cL_regressions/weighted_stimvals_' sub '.mat']);
...
for session = 1:nsessions
    ...
    ss = load([behavior_path 'food_choices_' sub '.mat']); % load all food choices
    nonmiss = ~isnan(ss.responses); % trials for which an answer was given
    ...
    for i = 1 : nrTrials
        ...
        if ~isnan(ss.responses(i))
            ...
            FV(i) = stimval.FV(i); % subjective food value on this trial
    ...
    ons_duration{1,session} = ss.timing.reaction_times(choice > 0 & nonmiss)'; % duration equals reaction time
    
    % define modulator values
    ons_modulate{1,session}{1}.vec = FV(choice > 0 & nonmiss); % actual vectors of what trial-by-trial values are
...
pmod(1).name = {'FV'}; % modulator for subjective weighted food value
pmod(1).param = {[ons_modulate{1,session}{1}.vec]}; % actual values of the parametric modulator
pmod(1).poly = {2}; % polynomial expansions, keep on 1 unless you want also quadratic effects (then set to 2)
