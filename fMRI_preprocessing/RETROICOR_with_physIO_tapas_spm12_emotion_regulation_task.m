...
% preprocessing_emotion_regulation.m so that we here can retrieve the motion 
...
for i=1:length(subs)
        ...
        % paths to raw data
        H_folder= ['/Users/username/mrdata/ESC/hrdata/' subs{i} filesep 'er/']; %folder in which the physio files from the scanner are stored
        D_folder= ['/Users/username/mrdata/ESC/subs/' subs{i} filesep 'functional/er/']; % folder with the functional scans for this task, er = emotion regulation
        OP_folder=['/Users/username/mrdata/ESC/hrdata/' subs{i} filesep 'er/physio_output/']; %where the output is saved
        mkdir(OP_folder)
        ...
        %below specify  the alinment of the scanner data to the log file - 
        ...
        matlabbatch{1}.spm.tools.physio.scan_timing.sqpar.Nscans = 681; % number of volumes that were acquired within the run
        ...
        matlabbatch{1}.spm.tools.physio.scan_timing.sqpar.Nprep = []; % count preparation pulses before 1 dummy- only fill if you use "first scan" instead of "last"
...
%         matlabbatch{1}.spm.tools.physio.model.movement.yes.outlier_rotation_deg = Inf;
        ...
        matlabbatch{1}.spm.tools.physio.model.output_multiple_regressors = ['er_multiple_regressors.mat']; % output file
